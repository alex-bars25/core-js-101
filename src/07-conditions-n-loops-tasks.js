...
function isTriangle(a, b, c) {
  return [a, b, c].every((item) => item !== 0) && a + b > c && a + c > b && b + c > a;
}
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  const numbers = [a, b].sort((c, d) => c - d);
...
function isCreditCardNumber(ccn) {
  const ccnStr = ccn.toString(10);
  const digits = ccnStr.length;
  let sum = 0;
...
function getDigitalRoot(num) {
  ...
  digits = digits.map((item) => +item);
  const root = digits.reduce((acc, curr) => acc + curr);
  return root.toString(10).length > 1 ? getDigitalRoot(root) : root;
}
...
function evaluateTicTacToePosition(position) {
  let lines = position.slice(0);
  lines.push(position.map((item) => item[0]));
  lines.push(position.map((item) => item[1]));
  lines.push(position.map((item) => item[2]));
  lines.push(position.map((item, idx) => item[idx]));
  lines.push(position.map((item, idx) => item[item.length - 1 - idx]));
  lines = lines.filter((item) => item.length === 3
    && item.every((el) => el === item[0]
      && el !== undefined));
  if (lines[0]) {
    return lines[0][0];
  }
